...
import {abi} from './abis/MuonDelegatorRewards.json';
...
const FILE_NAME = "./data/delegations.csv"
...
const missingContractAddress = () => {
  throw Error('Delegator address missing')
}

const main = async () => {
  const contractAddr: string = process.env.DELEGATOR_REWARDS_ADDRESS || missingContractAddress()
  ...
  let restakes: Array<boolean> = []

  records.map((row: Array<string>) => {
    addresses.push(row[0])
    balances.push(row[1])
    startDates.push(row[2])
    restakes.push(row[3].toLowerCase() == "true")
  })


  const contract = new web3.eth.Contract(abi, contractAddr)
  const tx = (contract.methods.bulkImport as any)(
    addresses, balances, startDates, restakes
  )
