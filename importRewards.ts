...
import {abi} from './abis/MuonRewardManager.json';
...
const FILE_NAME = "./data/rewards.csv"
...
const missingContractAddress = () => {
  throw Error('MuonRewardManager address missing')
}

const main = async () => {
  const contractAddr: string = process.env.MUON_REWARD_MANAGER || missingContractAddress()
  ...
  let rewardAmounts: Array<string> = []
  let tokenIds: Array<string> = []

  records.map((row: Array<string>) => {
    users.push(row[0])
    rewardAmounts.push(row[1])
    tokenIds.push(row[2])
  })
